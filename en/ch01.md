...
この関数のシグネチャーは、`BankAccount`関数がコンストラクターとして確実に呼び出されることを示すために、`new`キーワードがプレフィックスされています。関数型には関数とコンストラクタの両方のシグネチャーを持つ物があります。例えばJavaScriptにビルトインされているDateオブジェクトの型には、両方の種類のシグネチャが含まれています。
