...
クラスとインターフェースは、実装から切り離してソフトウエアコンポーネントの利用方法を記述するメカニズムを提供することによって、大規模なJavaScriptの開発をサポートします。TypeScriptはデザインタイムに(プライベートメンバーを利用できないようにすることで)クラス内部の実装をカプセル化を実践できますが、実行時には全てのオブジェクトプロパティはアクセス可能なため、実行時にカプセル化を強制することはできません。将来のJavaScriptでは実行時のプライベートメンバーを実現する*private names*が提供されるかもしれません。
...
現バージョンのJavaScriptで実行時にカプセル化を強制する方法は、クロージャー変数を用いてプライベートなフィールドとメソッドをカプセル化するモジュールパターンを用いることが唯一の方法です。モジュールパターンは組織的構造を提供する自然な方法であり、ソフトウエアコンポーネントの周辺に境界線を引くことによって動的ローディングを提供します。モジュールは名前空間を導入することも可能にすることで、多くのソフトウエアコンポーネントに提供されるグローバル名前空間を利用することが避けられます。

The following example illustrates the JavaScript module pattern.

以下の例はJavaScriptモジュールパターンを表しています。
...
この例ではモジュールパターンの*モジュールクロージャー*と*モジュールオブジェクト*の2つを例示しています。モジュールクロージャーはモジュールの実装をカプセル化する関数で、この例の場合では`Key`変数と`sendMessage`関数がカプセル化されています。モジュールオブジェクトはモジュールで公開される変数や関数を含みます。単純なモジュールは、モジュールオブジェクトを生成して返すだけかもしれません。上記のモジュールは`export`引数でobjectを受け取り、`sendMessage`プロパティをobjectに追加します。このような*augmentation*アプローチはモジュールの動的ローディングを単純化し、そのうえで複数のファイルにモジュールコードを分割することを可能にします。
...
この例では、レキシカルスコープ外部に`generateSecretKey`関数と`sendSecureMessage`関数が定義されており、外部スコープに`MessageModule`変数としてモジュールが代入されている事が仮定されています。
...
TypeScriptのモジュールは、モジュールパターンを簡潔に記述するための機構を提供します。TypeScriptではプログラマーは、外部モジュール内のクラスと入れ子モジュールを用いることで、モジュールパターンとクラスパターンを組み合わせることができます。

The following example shows the definition and use of a simple module.

以下の例は単純なモジュールの宣言と利用を示しています。
...
この例では、変数`s`はモジュールのプライベートで宣言されていますが、関数`f`はモジュール外部に公開されているので、モジュール外部のコードからアクセスが可能です。インターフェースや変数を用いてモジュール`M`について記述すると以下のようになります。
...
インターフェース`M`はモジュール`M`を外部から見た場合の振る舞いについてまとめています。TypeScriptでは、各レキシカルスコープは変数宣言空間と型宣言空間を持ち(詳細についてはセクション2.3を参照してください)、型の名前と変数の名前は衝突しないため、この例のようにインターフェースと初期化変数で同じ名前を使うことができます。
...
モジュールMは*外部*モジュール内にネストされているため(詳細はセクション10を参照してください)、これは*内部*モジュールの例になります。TypeScriptコンパイラーはこのモジュールを以下のようなJavaScriptとして出力します。
...
この場合、コンパイラはグローバル変数`M`にモジュールオブジェクトあると推論します。そして、それは要求するモジュールオブジェクトに初期化されたかもしれないし、されなかったかもしれません。
...
TypeScriptも、先頭行に*export*や*import*ディレクティブを含むファイルとして、*外部*モジュールをサポートします。この種のモジュールについては、Asynchronous Module Definitionシステムのような動的ローディングシステムの指定に応じて変更するモジュールクロージャーやモジュールオブジェクトの実装を、TypeScriptコンパイラは出力します。